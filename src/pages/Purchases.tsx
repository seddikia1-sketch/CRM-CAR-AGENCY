            ...
            {lines.map((line, idx) => (
              <div key={idx} style={{ border: '1px solid var(--border-color)', borderRadius: 12, padding: 12, marginBottom: 10, background: 'rgba(255,255,255,0.02)' }}>
                ...
                <div className="flex gap-md" style={{ flexWrap: 'wrap', marginTop: 8 }}>
                  ...
                  <Button variant="ghost" onClick={() => removeLine(idx)} leftIcon={<Trash2 size={14} />}>حذف السطر</Button>
                </div>
              </div>
            ))}
          </div>
          <Input label="ملاحظات الشحن" value={shippingNotes} onChange={(e) => setShippingNotes(e.target.value)} placeholder="ميناء، وكيل شحن..." />
          <div className="input-wrapper">
            <label className="input-label">ملاحظات عامة</label>
            <textarea className="input-field" style={{ minHeight: 60 }} value={notes} onChange={(e) => setNotes(e.target.value)} />
          </div>
        </div>
      </Modal>

      <Modal isOpen={supplierOpen} onClose={() => setSupplierOpen(false)} title="مورد الصين الرئيسي" maxWidth="520px"
        footer={<><Button variant="ghost" onClick={() => setSupplierOpen(false)}>إلغاء</Button><Button variant="primary" onClick={() => { updateSupplier(supForm); setSupplierOpen(false); setToast('تم حفظ بيانات المورد'); setTimeout(() => setToast(''), 2000); }}>حفظ</Button></>}
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <Input label="اسم المورد" value={supForm.name} onChange={(e) => setSupForm({ ...supForm, name: e.target.value })} />
          <Input label="الدولة" value={supForm.country} onChange={(e) => setSupForm({ ...supForm, country: e.target.value })} />
