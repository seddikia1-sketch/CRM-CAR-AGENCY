...
import {
  ...
  Truck, ShoppingCart, Wrench, Sun, Droplets, TrendingUp, HardDrive, Sparkles,
} from 'lucide-react';
import { getAiRequestHistory } from '../services/aiRequestHistory';
...
export const Dashboard: React.FC = () => {
  ...
  const aiHistory = useMemo(() => getAiRequestHistory().slice(0, 5), []);
      ...
      <div className="flex gap-md" style={{ alignItems: 'flex-start', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: '1 1 240px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div className="glass-card" style={{ padding: 'var(--spacing-md)' }}>
            <div className="flex justify-between items-center" style={{ marginBottom: 10 }}>
              <h3 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: 6 }}>
                <Sparkles size={18} color="#a89bff" /> آخر طلبات AI
              </h3>
              <Link to="/spare-parts" style={{ fontSize: '0.8rem' }}>قطع الغيار</Link>
            </div>
            {aiHistory.length === 0 ? (
              <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                لا طلبات بعد. استخدم «طلب قطعة ذكي» لحفظ السجل تلقائياً.
              </p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {aiHistory.map((log) => (
                  <div
                    ...
                    style={{
                      ...
                      background: log.error ? 'rgba(248,113,113,0.08)' : 'rgba(124,108,240,0.06)',
                    }}
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}>
                      <strong>{log.partQuery}</strong>
                      <span style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>
                        {new Date(log.createdAt).toLocaleString('ar-DZ', { dateStyle: 'short', timeStyle: 'short' })}
                      </span>
                    </div>
                    <div style={{ color: 'var(--text-secondary)', marginTop: 2 }}>
                      {[log.brand, log.model, log.year].filter(Boolean).join(' ') || '—'}
                      {log.vin ? ` · VIN: ${log.vin.slice(0, 10)}…` : ''}
                      {log.hasPhoto ? ' · 📷' : ''}
                    </div>
                    {log.error ? (
                      <div style={{ color: '#fca5a5', marginTop: 2, fontSize: '0.75rem' }}>خطأ: {log.error.slice(0, 60)}</div>
                    ) : (
                      <div style={{ color: '#a89bff', marginTop: 2, fontSize: '0.75rem' }}>
                        {log.suggestionsCount} اقتراح
                        {log.topSuggestions?.length ? ` · ${log.topSuggestions.slice(0, 2).join(' · ')}` : ''}
                      </div>
                    )}
                  </div>
                ))}
